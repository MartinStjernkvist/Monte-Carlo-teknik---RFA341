...
vävnad_sammansättning_vektor = np.array(
    [10.2, 12.3 / massa_C, 3.5 / massa_N, 72.9 / massa_O, 0.08 / massa_Na, 
     0.02 / massa_Mg, 0.2 / massa_P,0.5 / massa_S, 
     0.3 / massa_K, 0.007 / massa_Ca])
...
K_alpha_vektor = np.array(
    [K_alpha_H, K_alpha_C, K_alpha_N, K_alpha_O, K_alpha_Na, K_alpha_Mg, 
    K_alpha_P, K_alpha_S, K_alpha_K, K_alpha_Ca])
...
# Sönderfallsenergierna för Lu-177.
Lu177_energi = [208_366, 112_950, 321_316, 249_674, 71_642, 136_725]  
# Sönderfallsintensitet i % för respektive energi. Från laraweb.
Lu177_intensitet = [10.38, 6.2, 0.216, 0.2012, 0.1726, 0.047]  
# Kumulativa sannolikheten för sönderfall.
Lu177_sannolikhet = np.cumsum(Lu177_intensitet) / np.sum(Lu177_intensitet) 
...
def plot_stuff(x_data, y_data, scatter, label_data,
            marker='o', color='blue', x_label='x-label', y_label='y-label',
            title='1',fig_size=(10, 10), symbol_size=100, font_size=30, 
            alpha=1, line_width=10, x_lim=(0, 0), y_lim=(0, 0),
    ...
    if scatter == 0:
        plt.plot(x_data, y_data, color=color, alpha=alpha, linewidth=line_width, 
                 label=label_data)
    elif scatter == 1:
        plt.scatter(x_data, y_data, marker=marker, color=color, alpha=alpha, 
                    s=symbol_size, label=label_data)
    elif scatter == 2:
        plt.plot(x_data, y_data, color=color, alpha=alpha, linewidth=line_width, 
                 label=label_data)
        plt.scatter(x_data, y_data, marker=marker, color=color, alpha=alpha, 
        ...
        for i in range(len(x_data)):
            if scatter[i] == 1:
                plt.scatter(x_data[i], y_data[i], marker=marker[i],
                        color=color[i],alpha=alpha, s=symbol_size, 
                        label=label_data[i])
            else:
                plt.plot(x_data[i], y_data[i], color=color[i], alpha=alpha, 
                         linewidth=line_width, label=label_data[i])
...
class visualisera_matris:
    ...
    def __init__(self, array_3d, visa_något=False):
        ...
        # visa_något bestämmer ifall det som plottas ska ha olika färger 
        # beroende på voxelvärde, eller ifall allt som plottas ska vara 
        # vitt (så att det syns)
        if visa_något == False:
            self.vmin, self.vmax = 0, np.max(array_3d)
        else:
            self.vmin, self.vmax = 0, 0.0001 * np.max(array_3d)

        self.img1 = self.ax1.imshow(array_3d[self.slice_x_index, :, :], 
            cmap='gray', vmin=self.vmin, vmax=self.vmax)  # x-tvärsnitt
        
        self.ax1.set_title(f'X Slice: {self.slice_x_index}')
        self.img2 = self.ax2.imshow(array_3d[:, self.slice_y_index, :], 
            cmap='gray', vmin=self.vmin, vmax=self.vmax)  # y-tvärsnitt
        
        self.ax2.set_title(f'Y Slice: {self.slice_y_index}')
        self.img3 = self.ax3.imshow(array_3d[:, :, self.slice_z_index], 
             cmap='gray', vmin=self.vmin, vmax=self.vmax)  # z-tvärsnitt
    ...
    def create_sliders(self):
        ...
        self.slider_x = Slider(ax_slider_x, 'X Slice', 0, self.x - 1, 
                               valinit=self.slice_x_index, valstep=1)

        ax_slider_y = plt.axes([0.1, 0.06, 0.75, 0.03],
                                facecolor='lightgoldenrodyellow')
        self.slider_y = Slider(ax_slider_y, 'Y Slice', 0, self.y - 1, 
                               valinit=self.slice_y_index, valstep=1)

        ax_slider_z = plt.axes([0.1, 0.11, 0.75, 0.03], 
                               facecolor='lightgoldenrodyellow')
        self.slider_z = Slider(ax_slider_z, 'Z Slice', 0, self.z - 1,
                                valinit=self.slice_z_index, valstep=1)
...
# Genom att ansätta värden för njur-vävnad från filen Anatomidefinitioner.xlsx 
# kan en matris skapas som filtrerar bort alla voxelvärden från fantommatrisen 
...
"""
Samma grej som för njurarna, skillnaden är att vi byter target_values till 
värden med benmärg
"""
...
def energi_start(radionuklid_energi, radionuklid_sannolikhet):
    """
    Funktion som samplar den ursprungliga fotonenergin för varje ny foton 
    som sänds ut.
...
def medelvägslängd(mu):
    """
    Funktion som samplar steglängden utifrån den inverstransformerade 
    funktionen ovan.
    """
...
def ny_steg_transformera_koordinatsystem_3d(steg_A_B, phi_A, theta_A, 
                                            steg_B_C, phi_B, theta_B):
    """
    - Börjar på position A[x,y,z]- kalla detta koordinatsystem A.

    - Tar ett steg med steglängd steg_A_B, riktning (phi_A, theta_A), enligt
    koordinatsystemet i A. 
    (exempelvis kommer phi_A att vara relativt enhetsvektorn i x-led 
    för koord-syst A)

    - Tar ett steg till ny punkt - kalla denna punkt B.

    - Transformerar koordinatsystemet så att riktningsvektorn sammandfaller 
    med nya koordinatsystemet.
    (nya enhetsvektorn i x-led, i B's koord-syst, ska ha samma riktning 
    som fotonen hade när den tog steget)

    - Detta görs för att kunna sampla en ny riktning i nästa 
    växelverkanprocess, då behövs nämligen ett koordinatsystem i B.
    ...
    :return: 3 värden är förflyttningen från B till C, enligt A's 
    koord-syst
    """
    ...
    # för att x-axeln ska sammanfalla med riktningsvektorn måste 
    # rotationsvinkeln vara pi/2 - theta
    angle = -(pi / 2 - theta_A)
    R_y = np.array(
        [
...
@jit(nopython=True)
def bestäm_om_attenuerad(x_round, y_round, z_round, x_size, y_size, z_size, 
                         utanför_fantom, slicad_fantom_matris, foton_energi):
    """
    Funktion som bestämmer om en foton ska fortsätta följas eller inte.
    Termen "attenuerad" i koden kan även vara att fotonen rymt från 
    fantommatrisen.
    ...
    :param utanför_fantom: värde som ökar med 1, för att hålla reda på vad 
    som händer när koden körs
    :param slicad_fantom_matris: fantommatrisen
    :return: om attenuerad = 1 kommer fotonen att sluta följas
    """

    # Om positionen för fotonen är utanför fantommatrisen, indikera att 
    # fotonen ska sluta följas.
    if (
            x_round < 0
            or x_round >= x_size
            or y_round < 0
    ...
    # Om voxelvärdet för positionen är innanför matrisen, men befinner 
    # sig i luft (voxelvärde = 0)
    # eller
...
class attenueringsdata:
    ...
    def voxelvärde_till_material(self):
        ...
        Därför behövs en koppling mellan vävnaderna i anatomidefinitioner 
        och materialen i filen Attenueringsdata.xlsx.
        :return: 1) materialnamnet som motsvarar ett visst voxelvärde och 2) 
        en namnlista med material som ingår i fantomen.
        """
        # print(vävnad)
        ...
        # Skapa listor för materialen, där ett material motsvarar voxelvärden 
        ...
        big_list = [water, muscle, lung, dry_spine, dry_rib, blood, heart,
                    kidney, liver, lymph, pancreas, intestine, cartilage,
                    brain, spleen, air, breast_mammary, skin, eye_lens, 
                    red_marrow, yellow_marrow, thyroid, bladder]

        # Lista med namn, för att kunna indexera i attenueringsdatan.
        big_list_names = ['water', 'muscle', 'lung', 'dry_spine', 'dry_rib',
                        'blood', 'heart', 'kidney', 'liver','lymph', 'pancreas', 
                        'intestine','cartilage', 'brain', 'spleen', 'air', 
                        'breast_mammary', 'skin', 'eye_lens', 'red_marrow',
                        'yellow_marrow', 'thyroid','bladder']

        # Filtrerar en materiallista i taget i den större, sammansatta listan.
        # Om voxelvärdet matchar ett av värdena i en materiallista 
        # erhålls materialnamnet.
    ...
    def mu(self):
        """
        Funktion som tar fram attenueringskoefficienten utifrån energin och 
        voxelvärdet i en position.
        :return: attenueringskoefficient
        """

        # Energilistan (första kolumnen i Attenueringsdata.xlsx) läses in.
        # De två närmaste indexen, för energierna i listan som är närmast 
        # den ingående energin för funktionen tas fram.
        energi_list = np.array(self.energi_list)
        diff = np.abs(energi_list - self.energi)
        closest_indices = np.argsort(diff)[:2]

        # Kalla på ovanstående funktion, för att omvandla voxelvärde
        # till material.
        ...
        # Om närliggande attenueringskoefficienter är lika stora -> 
        # attenueringskoefficient = första värdet.
        if mu_close[1] - mu_close[0] < 10 ** (-15):
            mu_target = mu_close[0]

        # Om närliggande attenueringskoefficienter inte är lika stora 
        # -> linjärinterpolera fram attenueringskoefficienten.
        else:
            mu_target = mu_close[0] + (self.energi - energi_close[0]) * (
            mu_close[1] - mu_close[0]) / (
    ...
    def mu_max(self):
        """
        Funktion som tar fram den maximala attenueringskoefficienten 
        i fantomen för en viss energi.
        ...
        # Kalla på ovanstående funktion, för att erhålla namnlistan 
        # med material som ingår i fantomen.
        ...
        # attenueringskoefficienten för samma "närmaste index" som för energin.
        # En vektor med listor skapas.
        for i in range(len(big_list_names)):
            mu_array[i] = np.array(self.df_attenueringsdata[big_list_names[i]
                                                ][closest_indices].to_list())

        # Sortera bland vektorn med listorna, för att ta fram materialet 
        ...
        # Ta ut listan med de två attenueringskoefficienterna, för materialet
        # med högst attenueringskoefficient.
        ...
        #  -> linjärinterpolera fram attenueringskoefficienten.
            ...
            mu_max = mu_max_close[0] + (self.energi - energi_close[0]
            ) * (mu_max_close[1] - mu_max_close[0]) / (
            energi_close[1] - energi_close[0])
...
def bestäm_om_vxv(voxelvärde, energi, mu_max, df_attenueringsdata, 
    ...
    instans = attenueringsdata(voxelvärde, energi, df_attenueringsdata,
                                df_anatomidefinitioner)
    mu = instans.mu()
...
class växelverkan:
    ...
    def find_foto_tvärsnitt(self):
        ...
        if energi_close[1] - energi_close[0] < 10 ** (-15):
            ...
            foto_target = foto_close[0] + (self.energi - energi_close[0]
            ) * (foto_close[1] - foto_close[0]) / (
            energi_close[1] - energi_close[0])
        return foto_target
