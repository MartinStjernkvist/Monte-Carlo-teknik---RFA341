...
# Från: https://en.wikipedia.org/wiki/Gamma_ray_cross_section
r_e = np.sqrt(0.07941)  # sqrt(b): re2 = e4/Ee2 ≈ 0.07941 b 

# sqrt(b), bohr radius of hydrogen
a_0 = 5.29177210903 * 10 ** (-11) * 10 ** (14)  

#Ljusetshastighet
c = 3 * 10 ** 8

#Radie i meter enligt Physics Handbook 
radie_alpha = 1.2*10**(-15)*4**(1/3) 
...
vävnad_sammansättning_vektor = np.array(
    [10.2, 12.3 / massa_C, 3.5 / massa_N, 72.9 / massa_O, 
     0.08 / massa_Na, 0.02 / massa_Mg, 0.2 / massa_P,
...
K_alpha_vektor = np.array(
    [K_alpha_H, K_alpha_C, K_alpha_N, K_alpha_O, K_alpha_Na, 
    K_alpha_Mg, K_alpha_P, K_alpha_S, K_alpha_K, K_alpha_Ca])

# Viktat medelvärde
K_alpha = (1 / np.sum(vävnad_sammansättning_vektor
         )) * vävnad_sammansättning_vektor @ K_alpha_vektor.T
...
# sida 9 tabeller radiofysik del 1 canvas RFA331.
...
[fluorescence_yield_H, fluorescence_yield_C, fluorescence_yield_N, 
fluorescence_yield_O, fluorescence_yield_Na, fluorescence_yield_Mg, 
fluorescence_yield_P, fluorescence_yield_S, fluorescence_yield_K, 
fluorescence_yield_Ca])

# Viktat medelvärde.
fluorescence_yield = (1 / np.sum(vävnad_sammansättning_vektor
)) * vävnad_sammansättning_vektor @ fluorescence_yield_vektor.T

# Sidlängd på voxlarna i matriserna.
voxel_sidlängd = 0.15  # cm

# Sönderfallsenergierna för Lu-177 i eV. Från laraweb.
Lu177_energi = [208_366, 112_950, 321_316, 249_674, 71_642, 
                136_725]  

# Sönderfallsintensitet i % för respektive energi.
# Från laraweb.
Lu177_intensitet = [10.38, 6.2, 0.216, 0.2012, 0.1726, 0.047]  

# Kumulativa sannolikheten för sönderfall.
Lu177_sannolikhet = np.cumsum(Lu177_intensitet) / np.sum(
                                         Lu177_intensitet) 

#Energi i eV från Larawed
At211_energi = [5.869*10**6, 5.2119*10**6, 5.1403*10**6, 
                4.9934*10**6, 4.895*10**6]
...
def plot_stuff(x_data, y_data, scatter, label_data,
            marker='o', color='blue', x_label='x-label',
            y_label='y-label', title='1',fig_size=(10, 10), 
            symbol_size=100, font_size=30,alpha=1, 
            line_width=10, x_lim=(0, 0), y_lim=(0, 0),
    ...
    if scatter == 0:
        plt.plot(x_data, y_data, color=color, alpha=alpha, 
                 linewidth=line_width, label=label_data)
        
    elif scatter == 1:
        plt.scatter(x_data, y_data, marker=marker, color=color, 
                 alpha=alpha, s=symbol_size, label=label_data)
        ...
        for i in range(len(x_data)):
            if scatter[i] == 1:
                plt.scatter(x_data[i], y_data[i], 
                  marker=marker[i], color=color[i],alpha=alpha,
                    s=symbol_size, label=label_data[i])
            else:
                plt.plot(x_data[i], y_data[i], color=color[i], 
...
def end_time(start):
    ...
    if runtime < 60:
        ...
        print('Runtime: ' + str(round((runtime / 60), 1)) + 
                                                ' minutes')
...
class visualisera_matris:
    """
    Kod som visualiserar fantomen i tre olika tvärsnitt, med 
    sliders.
    """
    ...
    def __init__(self, array_3d, visa_något=False):
        ...
        # Initialisera mitt-tvärsnittet för respektive 
        # dimension.
        ...
        self.gs = GridSpec(1, 3, width_ratios=[1, 1, 1], 
                           height_ratios=[1])
        ...
        # visa_något bestämmer ifall det som plottas ska 
        # ha olika färger 
        # beroende på voxelvärde, eller ifall allt som 
        # plottas ska vara vitt (så att det syns)
        if visa_något == False:
            self.vmin, self.vmax = 0, np.max(array_3d)
        else:
            self.vmin, self.vmax = 0, 0.0001 * np.max(
                                            array_3d)

        self.img1 = self.ax1.imshow(array_3d[
            self.slice_x_index, :, :],cmap='gray', 
            vmin=self.vmin, vmax=self.vmax)  # x-tvärsnitt
        self.ax1.set_title(f'X Slice: {self.slice_x_index}')

        self.img2 = self.ax2.imshow(array_3d[:, 
          self.slice_y_index, :],cmap='gray', 
          vmin=self.vmin, vmax=self.vmax)  # y-tvärsnitt
        self.ax2.set_title(f'Y Slice: {self.slice_y_index}')

        self.img3 = self.ax3.imshow(array_3d[:, :,
             self.slice_z_index],cmap='gray', 
             vmin=self.vmin, vmax=self.vmax)  # z-tvärsnitt
    ...
    def create_sliders(self):
        ax_slider_x = plt.axes([0.1, 0.01, 0.75, 0.03],
                     facecolor='lightgoldenrodyellow')
        self.slider_x = Slider(ax_slider_x, 'X Slice', 0, 
         self.x - 1, valinit=self.slice_x_index,valstep=1)

        ax_slider_y = plt.axes([0.1, 0.06, 0.75, 0.03],
                        facecolor='lightgoldenrodyellow')
        self.slider_y = Slider(ax_slider_y, 'Y Slice', 0, 
         self.y - 1, valinit=self.slice_y_index, valstep=1)

        ax_slider_z = plt.axes([0.1, 0.11, 0.75, 0.03], 
                      facecolor='lightgoldenrodyellow')
        self.slider_z = Slider(ax_slider_z, 'Z Slice', 0,
            self.z - 1,valinit=self.slice_z_index, valstep=1)
    ...
    def update(self, val):
        ...
        self.img3.set_data(self.array_3d[:, :, 
                                         self.slice_z_index])
        self.ax3.set_title(f'Z Slice: {self.slice_z_index}')
...
# inkluderar fantom, avskuret vid benen
slicad_fantom_matris = fantom_matris[:, 34:212, 450:1182]  
...
# Genom att ansätta värden för njur-vävnad från filen 
# Anatomidefinitioner.xlsx kan en matris skapas som filtrerar
# bort alla voxelvärden från fantommatrisen 
# som inte motsvarar njurvävnad.
...
"""
Samma grej som för njurarna, skillnaden är att vi byter 
target_values till värden med benmärg
"""
...
def energi_start(radionuklid_energi, radionuklid_sannolikhet):
    """
    Funktion som samplar den ursprungliga fotonenergin för 
    varje ny foton som sänds ut.
    :param radionuklid_energi: Lista med fotonenergier som 
    är möjliga.
    :param radionuklid_sannolikhet: Lista med sannolikheten 
    för fotonergierna.
    :return: Slumpad energi
    """
...
def position_start(slicad_njure_matris):
    """
    Funktion som samplar en startposition utifrån en matris med
    endast njurvävnad.
...
def invers_funktion(x, mu):
    """
    Invers funktion, som används för att sampla fotonens 
    steglängd.
...
def medelvägslängd(mu):
    """
    Funktion som samplar steglängden utifrån den
    inverstransformerade funktionen ovan.
    """
    medelvägslängd = invers_funktion(np.random.rand(
                                    ), mu) / voxel_sidlängd  
...
def ny_steg_transformera_koordinatsystem_3d(steg_A_B, phi_A, 
                                            theta_A, steg_B_C, 
                                            phi_B, theta_B):
    """
    - Börjar på position A[x,y,z]- kalla detta koordinatsystem A.

    - Tar ett steg med steglängd steg_A_B, riktning (phi_A, 
    theta_A), enligt koordinatsystemet i A. 

    (exempelvis kommer phi_A att vara relativt enhetsvektorn i 
    x-led för koord-syst A)

    - Tar ett steg till ny punkt - kalla denna punkt B.

    - Transformerar koordinatsystemet så att riktningsvektorn 
    sammandfaller med nya koordinatsystemet.

    (nya enhetsvektorn i x-led, i B's koord-syst, ska ha samma 
    riktning som fotonen hade när den tog steget)

    - Detta görs för att kunna sampla en ny riktning i nästa 
    växelverkanprocess, då behövs nämligen ett koordinatsystem 
    i B.
    ...
    :return: 3 värden är förflyttningen från B till C, enligt 
    A's koord-syst
    """
    ...
    # för att x-axeln ska sammanfalla med riktningsvektorn 
    # måste rotationsvinkeln vara pi/2 - theta
    angle = -(pi / 2 - theta_A)
    R_y = np.array(
        [
    ...
    # först rotation i theta (y-axeln), sedan rotation i phi
    #  (z-axeln)
    R = np.dot(R_z, R_y)

    Homogenous_matrix = np.eye(4, dtype=np.float64)
    Homogenous_matrix[:3, :3] = R
    Homogenous_matrix[:3, 3] = np.array([dx_A_B, dy_A_B, 
                                  dz_A_B], dtype=np.float64)

    vektor_A_C = np.dot(Homogenous_matrix, np.array([dx_B_C,
                                    dy_B_C, dz_B_C, 1.0],
                                    dtype=np.float64))
...
@jit(nopython=True)
def bestäm_om_attenuerad(x_round, y_round, z_round, x_size, 
                         y_size, z_size, utanför_fantom, 
                         slicad_fantom_matris, foton_energi):
    """
    Funktion som bestämmer om en foton ska fortsätta följas 
    eller inte.

    Termen "attenuerad" i koden kan även vara att fotonen rymt
    från fantommatrisen.
    ...
    :param utanför_fantom: värde som ökar med 1, för att hålla 
    reda på vad som händer när koden körs
    :param slicad_fantom_matris: fantommatrisen
    :return: om attenuerad = 1 kommer fotonen att sluta följas
    """

    # Om positionen för fotonen är utanför fantommatrisen, 
    ...
    # Om voxelvärdet för positionen är innanför matrisen, men 
    # befinner sig i luft (voxelvärde = 0)
    # eller
    ...
    elif foton_energi < foton_energi_threshhold:
        utanför_fantom += 1
        attenuerad = 1  
    else:
        attenuerad = 0
...
class attenueringsdata:
    ...
    def __init__(self, voxelvärde, energi, df_attenueringsdata, 
                 df_anatomidefinitioner):

        # Läs in data från argumenten till klassen. 
        # Excelfilerna bearbetas med paketet pandas.
        ...
        self.energi_list = self.df_attenueringsdata['E'
                                                    ].to_list()

    def voxelvärde_till_material(self):
        """
        Funktion som omvandlar voxelvärdet i fantommatrisen 
        till ett material.

        Voxelvärdet motsvarar viss sorts vävnad, enligt filen 
        Anatomidefinitioner.xlsx.

        Därför behövs en koppling mellan vävnaderna i 
        anatomidefinitioner och materialen i filen 
        Attenueringsdata.xlsx.

        :return: 1) materialnamnet som motsvarar ett visst 
        voxelvärde och 2) en namnlista med material som ingår 
        i fantomen.
        """
        # print(vävnad)
        ...
        # Skapa listor för materialen, där ett material 
        # motsvarar voxelvärden 
        # (vävnad enligt Anatomidefinitioner).
        water = [3, 8, 12, 37]
        muscle = [6, 13]
        lung = [11]
        dry_spine = [27, 28]
        ...
        # Sätt ihop individuella materiallistor till en stor 
        #                                                lista.
        big_list = [water, muscle, lung, dry_spine, dry_rib,
                    blood, heart,kidney, liver, lymph,
                    pancreas, intestine, cartilage, brain, 
                    spleen, air, breast_mammary, skin, 
                    eye_lens, red_marrow, yellow_marrow, 
                    thyroid, bladder]

        # Lista med namn, för att kunna indexera i
        #                                   attenueringsdatan.
        big_list_names = ['water', 'muscle', 'lung', 
                        'dry_spine', 'dry_rib','blood', 
                        'heart', 'kidney', 'liver','lymph', 
                        'pancreas',  'intestine','cartilage',
                          'brain', 'spleen', 'air', 
                        'breast_mammary', 'skin', 'eye_lens',
                         'red_marrow','yellow_marrow', 
                         'thyroid','bladder']

        # Filtrerar en materiallista i taget i den större, 
        # sammansatta listan.

        # Om voxelvärdet matchar ett av värdena i en 
        # materiallista erhålls materialnamnet.
    ...
    def mu(self):
        """
        Funktion som tar fram attenueringskoefficienten utifrån
        energin och voxelvärdet i en position.

        :return: attenueringskoefficient
        """

        # Energilistan (första kolumnen i 
        # Attenueringsdata.xlsx) läses in.

        # De två närmaste indexen, för energierna i listan som
        # är närmast den ingående energin för funktionen tas fram.
        energi_list = np.array(self.energi_list)
        diff = np.abs(energi_list - self.energi)
        closest_indices = np.argsort(diff)[:2]

        # Kalla på ovanstående funktion, för att omvandla 
        # voxelvärde till material.
        ...
        # Om närliggande attenueringskoefficienter är lika 
        # stora -> attenueringskoefficient = första värdet.
        if mu_close[1] - mu_close[0] < 10 ** (-15):
            mu_target = mu_close[0]

        # Om närliggande attenueringskoefficienter inte är lika
        # stora -> linjärinterpolera fram attenueringskoefficienten.
        else:
            mu_target = mu_close[0] + (self.energi - energi_close[
            0]) * ( mu_close[1] - mu_close[0]) / (
    ...
    def mu_max(self):
        """
        Funktion som tar fram den maximala 
        attenueringskoefficienten i fantomen för en viss 
        energi.

        :return: maximal attenueringskoefficient
        """

        # Energilistan (första kolumnen i Attenueringsdata.xlsx)
        # läses in.

        # De två närmaste indexen, för energierna i listan som
        # är närmast den ingående energin för funktionen tas 
        # fram.
        energi_list = np.array(self.energi_list)
        diff = np.abs(energi_list - self.energi)
        closest_indices = np.argsort(diff)[:2]

        # Kalla på ovanstående funktion, för att erhålla 
        # namnlistan med material som ingår i fantomen.
        ...
        # För varje material i namnlistan tas värden på 
        # attenueringskoefficienten för samma "närmaste index"
        # som för energin.

        # En vektor med listor skapas.
        for i in range(len(big_list_names)):
            mu_array[i] = np.array(self.df_attenueringsdata[
                big_list_names[i]][closest_indices].to_list())

        # Sortera bland vektorn med listorna, för att ta fram 
        # materialet 
        # (indexet) som har störst attenueringskoefficient.
        mu_max_close_values = [np.max(arr) for arr in mu_array]
        mu_max_close = max(mu_max_close_values)
        mu_max_close_index = mu_max_close_values.index(
                                                mu_max_close)

        # Ta ut listan med de två attenueringskoefficienterna,
        # för materialet med högst attenueringskoefficient.
        mu_max_close = mu_array[mu_max_close_index]
        # Även energierna som ligger närmast energin som matas 
        # in i funktionen.
        energi_close = energi_list[closest_indices]
        ...
        #stora -> linjärinterpolera fram attenueringskoefficienten.
            ...
            mu_max = mu_max_close[0] + (self.energi - energi_close[
                0]) * (mu_max_close[1] - mu_max_close[0]) / (
            energi_close[1] - energi_close[0])
...
def bestäm_om_vxv(voxelvärde, energi, mu_max, df_attenueringsdata, 
                  df_anatomidefinitioner):
    
    instans = attenueringsdata(voxelvärde, energi, 
                        df_attenueringsdata,df_anatomidefinitioner)
    mu = instans.mu()
...
class växelverkan:
    ...
    def find_foto_tvärsnitt(self):
        ...
        if energi_close[1] - energi_close[0] < 10 ** (-15):
            ...
            foto_target = foto_close[0] + (self.energi - energi_close[
                0] ) * (foto_close[1] - foto_close[0]) / (
            energi_close[1] - energi_close[0])
        return foto_target
    ...
    def find_compton_tvärsnitt(self):
        ...
        if energi_close[1] - energi_close[0] < 10 ** (-15):
            ...
            compton_target = compton_close[0] + (self.energi - energi_close[
                0]) * (compton_close[1] - compton_close[0]) / (
                energi_close[1] - energi_close[0])
    ...
    def find_rayleigh_tvärsnitt(self):
        ...
        if energi_close[1] - energi_close[0] < 10 ** (-15):
            ...
            rayleigh_target = rayleigh_close[0] + (self.energi - energi_close[
                0])* (rayleigh_close[1] - rayleigh_close[0]) / (
             energi_close[1] - energi_close[0])
...
def foto_vxv(foton_energi):
    ...
    Antingen sker fluorescens eller inte, Augerelektroner antas deponera
     energi lokalt.
    :return: Energideponeringen som konsekvens av växelverkan, samt ifall
     fotonen slutar existera.
    """
    ...
    if np.random.rand() < fluorescence_yield:
        ...
        # Augerelektron, lokal energideponering -> finns ingen foton 
        # att följa.
...
def compton_vinkel_och_energiförlust(foton_energi):
    """
    Khans rejektionsalgoritm, artikeln "A Monte Carlo program for the 
    simulation of scintillation camera characteristics"

    Algoritmen samplar vinkeln och energideponeringen för en foton som
      Comptonväxelverkar.

    :return: Spridningsvinkeln theta, den spridda fotonens energi och 
    den spridda fotonens energiförlust (=energideponering).
    """
    ...
    while True:
        ...
        # Termen alpha är fotonens energi i termer av vilomassan (energi)
        #  för en elektron.
...
def run_MC_multiprocess(args):
    (start, end, tvärsnitt_file, attenueringsdata_file, 
     anatomidefinitioner_file, slicad_fantom_matris,
     slicad_njure_matris, slicad_benmärg_matris, voxel_sidlängd, 
     radionuklid_energi, radionuklid_intensitet,
    ...
    # Matrisdimensionerna, viktigt för att bedöma ifall fotonen är 
    # innanför matrisen eller inte.
    x_size, y_size, z_size = slicad_fantom_matris.shape

    # Skapa tom matris, för att sedan fylla på med energideponering 
    # i voxklarna.
    ...
    for i in range(start, end):
        ...
        instans = attenueringsdata(voxelvärde, foton_energi,df_attenueringsdata, 
                                   df_anatomidefinitioner)
        mu_max = instans.mu_max()

        vxv_sker = False
        while vxv_sker == False:
            # Sampla medelvägslängden från inverstransformerad 
            # attenueringsfunktion.
            ...
            # Eftersom voxlarna har diskreta positioner måste avrundning till 
            # närmaste heltal göras.
            dx, dy, dz = steg(theta, phi, steglängd)
            x, y, z, x_round, y_round, z_round = förflyttning(x_start, y_start,
                                                          z_start, dx, dy, dz)

            #   -----------------------------------------------------------------
            #   Om foton hamnar utanför fantommatrisen -> kasta ut foton 
            #   ur loopen
            #   Ekvationen förekommer nedan efter varje nytt steg tas, 
            #   dock utan if-statement.
            #   -------------------------------------------------------------------

            attenuerad, utanför_fantom = bestäm_om_attenuerad(x_round, y_round,
                            z_round, x_size, y_size, z_size, utanför_fantom, 
                            slicad_fantom_matris, foton_energi)

            if attenuerad == 0:
                voxelvärde = slicad_fantom_matris[x_round, y_round, z_round]

                vxv_sker = bestäm_om_vxv(voxelvärde, foton_energi, mu_max,
                           df_attenueringsdata, df_anatomidefinitioner)
            else:
                # utanför matris
                vxv_sker = True
    ...
    #   ----------------------------------------------------------------------
            while attenuerad == 0:
                ...
                voxelvärde = slicad_fantom_matris[x_round, y_round, z_round]
                instans = attenueringsdata(voxelvärde, foton_energi, 
                                           df_attenueringsdata, df_anatomidefinitioner)
                mu_max = instans.mu_max()
                ...
                if vxv == 'foto':
                    ...
                    # Registrera energideponering ifall fotonen
                    # växelverkar i en voxel med benmärg.
                    if slicad_benmärg_matris[x_round, y_round,
                                              z_round] != 0:
                        träff_benmärg += 1

                        benmärg_matris_deponerad_energi[x_round,
                           y_round, z_round] += energi_deponering

                        print(
                            f'foto: {energi_deponering:.0f} eV i voxel 
                    ...
                    if attenuerad == 0:
                        ...
                        vxv_sker = False
                        while vxv_sker == False:
                            # Sampla medelvägslängden från 
                            # inverstransformerad attenueringsfunktion.
                            steglängd_foto = medelvägslängd(mu_max)

                            dx, dy, dz = steg(theta, phi, 
                                              steglängd_foto)
                            
                            x, y, z, x_round, y_round, z_round = förflyttning(
                                x, y, z, dx, dy, dz)

                            # Om foton hamnar utanför 
                            # fantommatrisen -> kasta ut 
                            # foton ur loopen.
                            attenuerad, utanför_fantom = bestäm_om_attenuerad(
                            x_round, y_round, z_round,x_size, 
                            y_size,z_size,utanför_fantom, 
                            slicad_fantom_matris,foton_energi)

                            if attenuerad == 0:
                                voxelvärde = slicad_fantom_matris[
                                    x_round, y_round, z_round]

                                vxv_sker = bestäm_om_vxv(
                                    voxelvärde, foton_energi,
                                      mu_max,df_attenueringsdata,
                                      df_anatomidefinitioner)
                            else:
                                vxv_sker = True

                        # Ingångsvärden till koordinat-transformeringen
                        # som behöver genomföras ifall 
                        # växelverkan = compton eller rayleigh.
                ...
                #   -----------------------------------------
                    ...
                    # Sampla spridningsvinklar, samt 
                    # energideponeringen vid Comptonväxelverkan.
                    ...
                    if slicad_benmärg_matris[x_round, y_round,
                        ...
                        benmärg_matris_deponerad_energi[ x_round, 
                        y_round, z_round
                        ] += energideponering_compton

                        print(
                            f'compton: {energideponering_compton:.0f} 
                            eV i voxel [{x_round, y_round, z_round}]')
                    ...
                    while vxv_sker == False:
                        ...
                        # Koordinattransformation, eftersom 
                        # spridningsvinklarna för 
                        # Comptonspridning inte kan samplas
                        #  uniformt.
                        dx_compton, dy_compton, dz_compton = ny_steg_transformera_koordinatsystem_3d(
                        steglängd, phi , steglängd_compton
                        ,phi_compton,theta_compton)

                        # Ta ett nytt steg.
                        x, y, z, x_round, y_round, z_round = förflyttning(
                            x, y, z, dx_compton, dy_compton,
                              dz_compton)

                        # Om foton hamnar utanför
                        #  fantommatrisen -> kasta ut foton ur loopen.
                        attenuerad, utanför_fantom = bestäm_om_attenuerad(
                            x_round, y_round, z_round,
                        ...
                        if attenuerad == 0:
                            ...
                            vxv_sker = bestäm_om_vxv(voxelvärde,
                            foton_energi, mu_max, df_attenueringsdata,
                                 df_anatomidefinitioner)
                        else:
                            vxv_sker = True

                    # Ingångsvärden till koordinat-transformeringen 
                    # (om nästa växelverkan är Comptonspridning eller 
                    # Rayleighspridning).
                ...
                #   -----------------------------------------
                    ...
                    while vxv_sker == False:
                        ...
                        # Koordinattransformation, eftersom 
                        # spridningsvinklarna för 
                        # Rayleighspridning inte kan samplas 
                        # uniformt.
                        ...
                        # Ta ett nytt steg.
                        x, y, z, x_round, y_round, z_round = förflyttning(
                            x, y, z, dx_rayleigh, dy_rayleigh,dz_rayleigh)

                        # Om foton hamnar utanför fantommatrisen 
                        # -> kasta ut foton ur loopen.
                        attenuerad, utanför_fantom = bestäm_om_attenuerad(
                         x_round, y_round, z_round, x_size, y_size,
                    ...
                    # Ingångsvärden till koordinat-transformeringen 
                    # (om nästa växelverkan är Comptonspridning 
                    # eller Rayleighspridning).
                    theta, phi = theta_rayleigh, phi_rayleigh
                    steglängd = steglängd_rayleigh

    # Några print statements, för att hålla reda på vad som 
    # händer när koden kör.
    print(f'max värdet av matrisen: 
                {np.max(benmärg_matris_deponerad_energi)}')
...
def inputs_riktig_körning():
    ...
    if input_standard == 's':
        ...
        if eval(input_dummy_magnitud_iterationer) > 4:
            ...
            iterationer_dummy = 10 ** (eval(
                            input_dummy_magnitud_iterationer))
        ...
        if eval(input_riktig_magnitud_iterationer) >= 8:
            ...
            iterationer_tot = eval(input_riktig_skalär_iterationer
                ) * 10 ** ( eval(input_riktig_magnitud_iterationer))

    print('antal_cores, iterationer_dummy, iterationer_tot: ', 
          antal_cores, iterationer_dummy, iterationer_tot)
...
def spara_resultat(matris, json_object):
    ...
    if input_spara_resultat == 's':
        ...
        # Spara resultatmatrisen i en numpy fil, som sedan går 
        # att visualisera i en separat fil.
    ...
    return print(f'Resultat sparade i filerna [{fil_namn_npy}] 
                        och [{fil_namn_json}]')
...
if __name__ == "__main__":
    ...
    # Lite kod för att dela upp arbetet i flera processer, fördelat 
    # på olika processor-kärnor.
    chunk_storlek = iterationer_dummy // antal_cores
    chunk_ranges = [(i * chunk_storlek, (i + 1) * chunk_storlek
                     ) for i in range(antal_cores)]
    
    chunk_ranges[-1] = (chunk_ranges[-1][0], iterationer_dummy)

    # Inbakade argument för funktionen.
    args_packed = [(start, end, tvärsnitt_file, attenueringsdata_file, 
                    anatomidefinitioner_file, slicad_fantom_matris,
                    slicad_njure_matris, slicad_benmärg_matris,
                    voxel_sidlängd, radionuklid_energi,
                    radionuklid_intensitet, radionuklid_sannolikhet
    ...
    with mp.Pool(antal_cores) as pool:
        partial_results = pool.map(run_MC_multiprocess, 
                                             args_packed)

    # Summera resultatmatriserna från respektive process.
    benmärg_matris_deponerad_energi = np.sum(partial_results, 
                                                    axis=0)
...
def visualisera_resultat():
    ...
    if input_resultat == 's':
        ...
        visualisera = visualisera_matris(resultat_matris, 
                                         visa_något=True)
        visualisera.show()

        slicad_resultat_matris = resultat_matris[105:160, 
                                                 80:, 140:630]
        
        visualisera = visualisera_matris(slicad_resultat_matris, 
                                         visa_något=True)
        visualisera.show()
        ...
        slicad_resultat_matris = resultat_matris[105:160, 80:, 
                                                 140:630]
    ...
    return print(
        f'BENMÄRG: Energideponering per foton (eV / sönderfall):
          {np.sum(resultat_matris) / iterationer_tot:.1f} 
          \nRYGGRAD: Energideponering per fotoN (eV / sönderfall): 
          {np.sum(slicad_resultat_matris) / iterationer_tot:.1f}')
